...
const updateProgress = () => {
  document.querySelector('#progress').textContent = `${Math.round(
    calculateProgress(todos) * 100
  )}%`
}
...
const findMostImportantTodo = () => {
  document.querySelector('#important-todo').textContent = todos.find(
    t => t.priorityValue === minPriority
  ).title
}
...
const findLastTodo = () => {
  document.querySelector('#less-important-todo').textContent = todos.find(
    t => t.priorityValue === maxPriority
  ).title
}
